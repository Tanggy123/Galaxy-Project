...
class Model {
    /**
     * a 2d array data structure for model.
     */
    private int[][] board;
    ...
    void copy(Model model) {
        if (model == this) {
            ...
            for (int a = 0; a < xlim(); a += 1) {
                for (int b = 0; b < ylim(); b += 1) {
                    this.board[a][b] = model.board[a][b];
                }
            }
        }
    }
    ...
    boolean isBoundary(int x, int y) {
        return isEdge(x, y) && (board[x][y] == 1 || board[x][y] == 100 + 1);
    }
    ...
    /**
     * Returns true iff P is a stray boundary.
     */
    boolean isStrayBoundary(Place p) {
        if (isBoundary(p) && isHoriz(p)) {
            if ((!isBoundary(p.move(-2, 0))
                    && !isBoundary(p.move(-1, 1))
                    && !isBoundary(p.move(-1, -1)))
                    || (!isBoundary(p.move(2, 0))
                    && !isBoundary(p.move(1, 1))
                    && !isBoundary(p.move(1, -1)))) {
                return true;
            }
        ...
        if (isBoundary(p) && isVert(p)) {
            if ((!isBoundary(p.move(0, 2))
                    && !isBoundary(p.move(-1, 1))
                    && !isBoundary(p.move(1, 1)))
                    || (!isBoundary(p.move(0, -2))
                    && !isBoundary(p.move(-1, -1))
                    && !isBoundary(p.move(1, -1)))) {
                return true;
            }
    ...
    private boolean isGalaxy(Place center, HashSet<Place> region) {
        for (Place cell : region) {
            ...
            for (int i = 0; i < 4; i += 1) {
                ...
                if (region.contains(nextCell) && isBoundary(edge)) {
                    return false;
                }
                if (isBoundary(edge)) {
                    if (isStrayBoundary(edge)) {
                        return false;
                    }
                    if (!isBoundary(pl(2 * center.x - edge.x,
                            2 * center.y - edge.y))) {
                        return false;
                    }
                }
                if (isCenter(edge) && !edge.equals(center)) {
                    return false;
                }
            }
            for (int i = 0; i < 4; i += 1) {
    ...
    Place opposing(Place p0, Place p) {
        if (!isCell(2 * p0.x - p.x, 2 * p0.y - p.y)) {
            return null;
        }
        return pl((2 * p0.x - p.x), (2 * p0.y - p.y));
    }
    ...
    List<Place> unmarkedSymAdjacent(Place center, List<Place> region) {
        ...
        for (Place r : region) {
            assert isCell(r);
            for (int i = 0; i < 4; i += 1) {
                int moveX = (i % 2) * (2 * (i / 2) - 1) * 2,
                        moveY = ((i + 1) % 2) * (2 * (i / 2) - 1) * 2;
                Place p = r.move(moveX, moveY);
     ...
     */
    List<Place> centers() {
        ArrayList<Place> centers = new ArrayList<>();
        for (int a = 0; a < xlim(); a += 1) {
            for (int b = 0; b < ylim(); b += 1) {
                if (isCenter(a, b)) {
                    centers.add(pl(a, b));
